...
### Fixed
- **`csb scan -d/-D <gone-path>` now searches the index.** The missing-path check warned and returned -- or, with a term, silently fell back to a BROAD term search, abandoning the path scope -- without ever running the query. Existence acting as a filter, in the flag whose purpose is finding history: the index is the only place a vanished folder's history lives, and "the folder is gone" describes the exact situation the `--deleted --restore` recovery flow exists for. Now it warns and continues; gone-path + term stays path-scoped and term-filtered.
- **Relative `cd` targets resolve instead of leaking as folder names.** `cd -`, `cd ..`, `cd foo` produced bare-word "folders" -- a literal `-` carried work credit in three real sessions, from the common `cd "$BR"; ...; cd - >/dev/null` loop idiom. A relative target now resolves lexically against the incoming cwd (`cd ..` -> the parent, `cd foo` -> `cwd\foo`); `cd -` ("previous directory", unrecorded in the transcript) is discarded.
- **Primary credit ranks by certainty of evidence, not syntax of mention.** An explicit `cd` is a *fact* -- the shell demonstrably moved there. An absolute path in command text is a *guess*: operand or string literal, indistinguishable without executing the command, and 6 real work units had been credited to `C:\code\x`, a string inside a `python -c` payload. When a command cd's somewhere, only paths the move corroborates (inside the target's subtree, or resolved relative to it) outrank the target; everything else demotes to presence. Measured against all 4,532 command calls in a real vault before adoption -- the decisive case was a literal inside `sed` replacement text that happens to EXIST on disk, which neither payload parsing nor any existence rule could catch. Without a `cd`, ranking is unchanged: promoting the ambient cwd the same way would re-flatten work onto the launch directory (928 measured commands), which is the original #56 bug. Existence still plays no role in primary resolution: a deleted repo named as a plain operand keeps its credit.
- **Path extraction returns source order.** It concatenated all Windows-form matches ahead of all POSIX-form matches while its docstring claimed "in order of appearance" -- and "the first path mentioned" is what primary resolution is built on, so a command mixing spellings could mis-assign its work. All patterns now merge on match position.
- **Quoted Windows paths keep their spaces.** `"C:\Program Files\App"` truncated at the space even when quoted, shedding junk fragments (`C:\PROGRAM`, `C:\PYTHON`, `C:\USERS`, `C:\WINDOWS` -- all measured in a real index). A quote is the boundary the bare pattern lacks; unquoted spaced paths still truncate (no boundary to trust), but their fragments are nonexistent and the display tier now hides them.
- **Dot-names are decided by a directory allowlist, never the extension heuristic.** Foreign transcripts broke both ways: `.gitignore` (9 letters) exceeded the 6-char extension cap and stored whole as a "folder" whenever the file no longer existed locally, while `.git` FITS the cap, classified as a file, and was dirnamed away. Known dot-directories (`.git`, `.github`, `.venv`, ...) are kept; unknown dot-names default to file -- a wrong "file" mis-credits by one level, a wrong "folder" fabricates a place users see in rankings. The filesystem still overrides both whenever the path exists.
- **The presence tier now reads the existence hint it already stores.** Schema v7 records `path_exists` and the display classifier has consulted it from day one -- but every folder SELECT omitted the column, so a nonexistent path could never classify as low-value and test-fixture literals (`C:\code\hot-1`, `\home\a`) rendered at full prominence in "Also touched". The classifier was right; the query starved it. Measured after: the reference session's collapsed tier went from 16 paths to 53, all junk. Display-only as always: the rows stay indexed and findable by `csb scan`, and NULL (probe failed) still renders normally -- unknown is not missing.
- **Backtick joins the rejected metacharacters.** A PowerShell escape / POSIX substitution character adjacent to a path was absorbed into the match; a real index carried ``C:\Users\Extreme` `` with a work unit. (8.3 short names like `PROGRA~1` use `~`, not a backtick, and are unaffected.)
- **Repeated `--filter` keys: the strictest wins.** `--filter min-work=50 --filter min-work=5` silently discarded the stricter constraint (`dict()` kept the last pair). Repeated filters AND together; for a threshold, AND means max. And `min-work=-5` -- previously a silent no-op -- is now rejected as not non-negative.
- Stale `-d` help text still described the removed top-N match gate.
- **Quoted `cd`/`pushd` targets keep their spaces** (third adversarial pass, the one new SEV-1 it found). The quoted-path fix had landed in general extraction but never reached the parallel cd-target parser, so `cd "C:\Program Files\App"` truncated its base to `C:\Program` -- and because an explicit cd is ranked as a *fact*, the fabricated fragment won primary credit while the real target failed the corroboration prefix test against the wrong base and demoted to presence. Latent in this vault only because its paths carry no spaces; deterministic the moment any session cd's into `Program Files` or a "First Last" profile dir. `pushd`, `cd /d`, and single-quote forms covered.
- **`--filter`'s bad-value error names what to type, not a Python identifier.** The negative-floor fix briefly made it read `expects _nonneg_int, got 'lots'`; it now says `expects a non-negative integer`.
- **Quotes protect every path spelling, not just drive-letter form** (user-found, in their own vault). The quoted-path fix matched `"C:\..."` only; quoted Git-Bash/WSL and POSIX spellings fell to the bare pattern, whose comma boundary (a deliberate prose-list guard -- `see C:, C:` must never glue) sliced `"/d/M/Software/_DeDRM, Proxies, Security, and Scripts"` down to a fragment carrying 14 real work units. The `[not found]` marker surfaced it -- the display safety net working as designed. Quoted RELATIVE tokens now feed the existence-gated resolver too, so `cd /d/M && python "Software/_DeDRM, .../x.py"` credits the real deep folder instead of stopping at `Software`. Verified healed in the live index: the fragment is gone and the credits moved to the actual script subfolders. Third instance of a fix landing at one call site while its parallel was missed -- the adversarial pass explicitly hunted for a fourth and found none.
- **All-dots path components rejected.** `C:\Users\...` -- prose using "..." as et-cetera inside a command -- was stored as a verified-existing worked folder, because Win32 strips trailing dots and the existence probe resolved it to the real `C:\Users`. The one junk shape measured to *defeat* the existence safety net, so it is rejected lexically, before any probe. Interior dots (`a...b`, `release.v2`) are untouched.

### Added
- **Prominent scratch escapes the low-value collapse.** The unconditional scratch-is-noise rule misrepresented sessions whose scratchpad IS the workplace -- measured, one real session's #1 folder (its Claude Code scratchpad, 207 work units) was invisible behind "(N hidden)". A scratch folder now surfaces in the ranked tier, tagged `(scratch)`, when it carries more than 10 work units or ranks in the session's top 10 -- both arms measured against every session before adoption (the absolute floor catches material buried in busy sessions; the relative arm catches sessions that live in scratch). For purge-prone scratchpads this is a recovery affordance: those dirs hold the session's probe scripts and tools, and real vaults show them reused across sessions. Zero-work scratch stays collapsed -- the escape is earned by work.
- **The collapse always states its aggregate.** The hidden-tier note now reports total hidden work units, and `--all` shows per-row counts sorted most-material-first -- mirroring `--filter`'s never-hide-the-total rule.

### Tests
- **1289 pass, 1 skipped** (was 1235 at the 0.7.1 baseline; +54). Every fix red-green verified; the ranking change ships with its measurement (`measure_cd_ranking` results summarized in the design ledger) and pins the invariants that must NOT change: the #56 subfolder fix, deleted-repo history, and no-cd ranking.
...
### Fixed
- **Work locations are harvested from tool calls, not just `cwd`.** Claude Code stamps every event with the directory the session was *launched* from and follows it only into descendants -- never across repositories. A session that edits, tests, and releases another repo therefore recorded exactly one folder. The information was in the transcript all along (795 mentions in the reference session), just not in the field the indexer read. `csb scan C:\code\other-repo` now finds those sessions.
- **`usage_count` counts WORK UNITS, not transcript events.** The old measure gave one tick per event, and the top `cwd` value is 69-96% of every record in a transcript -- so it measured session *length*, not folder preference, and the launch directory outranked the real workplace **by construction** (1201 ticks against 226 tool calls in the reference session). One tool call is now one unit of work, credited to the folder that call actually operated in. Reference session: `C:\code\dazzle-filekit` 55 vs the launch dir's 24 -- previously 0 vs 1201.
- **Relative paths after a `cd` are resolved.** `cd repo && pytest tests/x.py` credited only `repo`, burying every subfolder. Measured: 74% of commands containing a `cd` also name a relative path. Resolving them moved the reference session's root count from 123 to 55 as credit redistributed to `tests/`, `docs/`, `dazzle_filekit/`, and `private/**`. The `cd` argument itself is deliberately *demoted* -- it says where the shell stands, not what the call worked on.

### Added
- **Presence vs. work as separate questions.** Every folder a call touched gets a row (so `csb scan` finds it), while only the call's *primary* folder is credited with work. Measured: 33% of touched folders are never any call's primary, so crediting-only would have recreated the bug at smaller scale.
- **Three display tiers in `csb show`** -- work locations (ranked), "Also touched (no work credited)" (no misleading `0x`), and a collapsed low-value tier for scratch directories and unresolved paths.
- **`csb show --filter min-work=N`** hides working directories below N work units, always reporting how many were hidden and their total. **`csb show --all`** reveals the low-value tier. (Deliberately *not* `--all-folders`, which means "no top-N truncation" on `list`/`scan` -- one flag, one meaning.)
- **`--filter` takes `KEY=VALUE` and is repeatable**, so narrowing has one vocabulary instead of a new flag per predicate. The keyword positional on `list`/`tree` is the shorthand for the common case. The comparison direction lives in the key (`min-work=10`) rather than an operator (`work>=10`), because `>` and `<` are redirection in cmd.exe and PowerShell and get eaten before csb sees them. An unknown key is a hard error naming the valid ones -- a silently ignored filter is a wrong answer wearing a right answer's clothes.
- **Schema v7** (purely additive): `folder_usage.path_exists` / `.path_verified_at` record whether a folder still resolves -- a confidence *hint*, never a filter, since a folder that was genuinely worked in and later deleted is real history. `sessions.tool_paths_extracted_at` is NULL until harvesting has run, so "unmeasured" can never render as "zero folders".
- Five precision rules keep non-places out of the index: anchored path matching, a POSIX root allowlist, device rejection (via `dazzle_filekit.is_device_path`), shell-metacharacter rejection, and empty-component rejection. Each was added in response to junk found in **real** transcripts -- `2>/dev/null` ranked #1 at 119 hits under the naive approach; `C:\code\$r` (an unexpanded variable) appeared 62 times; Python string literals from another project's adversarial tests parsed as paths.

- **`--top N` no longer decides which sessions exist.** `csb scan` passed its display cap into the folder-match query, where it doubled as a gate: a session only matched a path if that path was among its busiest few folders. Harmless while a session recorded ~1 folder -- and *actively rationalized* in a code comment as a deliberate difference from `csb tree` -- it hid roughly two thirds of real results once every touched folder was recorded (`-d C:\Users\Extreme` returned 4 sessions where 14 matched). How many folders we print and whether a session matches are now different numbers. **Behavior change:** `-d`/`-D` return strictly more sessions than in 0.7.0; `--top` and `--all-folders` affect display only.
- **`csb scan -D` no longer labels itself a descendant search.** It printed "sessions **under** `<path>`" -- `-d`'s wording -- for a search that deliberately excludes descendants. Wildcard form now marks itself `(no descendants)` too, so `-d *` and `-D *` no longer print identical headers for different questions.
- **`csb scan -D` no longer RETURNS descendants either.** The query has two halves -- a filesystem pass and a SQL pass -- and only the SQL half honored `-D`. The filesystem pass matches Claude Code's sanitized project-directory names by prefix, where `C:\code` becomes `C--code` and therefore also matches `C--code-notepad-cleanup`, a child directory. Measured on a real index: `csb scan -D "C:\code"` returned 11 sessions where 10 had the exact folder, the extra one's only connection being a start folder one level below. `-D` now means what it documents.
- **Hint text told users to type flags that do not exist.** `csb show`'s hidden-tier note said `--all-folders to show`, which `show` does not accept (`unrecognized arguments`), and its thin-folder note said `--filter 0`, which is no longer valid syntax. Both are now runnable, and a test walks every flag a hint prints and asserts the parser accepts it, so the next stale hint is caught automatically.
- **`csb show --all` no longer shares an argparse dest with `--all-folders`.** They mean different things on different verbs but both wrote to `all_folders`, so any shared render path would have read one as the other. Latent rather than live, and now impossible.
- **Files were being stored and ranked as working directories.** Measured on a real index: 38 stored folders were files on disk and **30 carried credited work** -- `dazzlesum.py` ranked as a working directory with 36 units. Two independent causes. `Glob`/`Grep`'s `path` argument was taken verbatim, and `Grep` accepts a single file (`Read`/`Edit` already reduced theirs to the parent, which is why only this branch leaked). And the file test was a trailing-extension guess, which misses every extensionless file -- `VERSION`, `Makefile`, a git `pre-commit` hook. The question is now put to the filesystem via **`dazzle_filekit.classify_fs_object`**, falling back to the lexical guess only when a path cannot be classified, since a historical transcript routinely names paths that no longer exist.
- **One folder could be ranked twice under two spellings.** `..` and bare `.` components were never collapsed, so `MoonshotAI\..\DeepSeek` competed with `DeepSeek` and split its credit -- and it landed in the *ranked* tier, not the collapsed one. Collapsing is purely lexical (`ntpath`, so it behaves identically on POSIX hosts where `os.path` would not treat `\` as a separator at all).
- **Two new precision rules**, both from junk found in a real index: a colon past the drive spec (a colon-joined `PYTHONPATH` list flattened into one string, a pytest node id like `test_x.py::TestClass`, prose from a diagnostic `echo`), and a leading `~` (unresolvable for a transcript that may come from another machine or user -- a bare `~` had accumulated 26 work units while the same directory was also stored correctly expanded). Windows 8.3 short names like `PROGRA~1` are deliberately unaffected, since the `~` is inside a component rather than leading it.
- **The file/directory probe now runs on the normalized spelling.** It had been asking the filesystem about whatever spelling the command used, and Windows cannot resolve the Git Bash or WSL forms -- `Path("/d/M/.gitignore").exists()` is False even when `D:\M\.gitignore` is right there. Every drive-mapped POSIX path therefore answered "nonexistent" and fell through to the lexical guess, which is exactly where dotfiles slip: `.gitignore` has no extension the guess recognizes, and it cannot be added to the known-filenames list either, because `.git`, `.venv`, and `.github` share its shape and are directories. Only the filesystem can tell those apart, so it has to be asked in a spelling it understands.
- Together these reject or correct **every junk value** measured in a real index -- files stored as folders 38 -> 0, unresolved `..` 3 -> 0, bare `.` components 5 -> 0, stray colons 7 -> 0. **`csb update rebuild-index` is required to clean rows already stored.**

### Changed (CLI)
- **BREAKING: `csb scan --no-usage` / `-NU` is now `--no-index` / `-NI`.** The old names are removed, not aliased. They described a *side effect* -- "only consider start_folder" -- rather than what the flag does, which is bypass the SQLite index and find sessions by walking `~/.claude/projects/` and reading transcripts. That mattered little while `folder_usage` held one folder per session; this release made the index rich enough that the side-effect description read as a duplicate of `-s`, obscuring the flag's actual purpose as the escape hatch for a stale or half-built index. Its help now leads with the mechanism, and warns that it cannot find deleted sessions. Scripts using `-NU` will fail loudly with `unrecognized arguments` rather than silently changing behavior.
- **`-NI` with `--deleted` now explains itself.** Deleted sessions exist only in the index, and `-NI` bypasses the index, so the combination can never match -- verified against a scratch index containing a deleted session. Instead of a bare "no sessions found" plus a generic tip, csb now says why: *"-NI cannot find deleted sessions -- they exist only in the index, which -NI bypasses. Drop -NI."*
- **`csb scan`, `csb list`, and `csb tree` group their flags in `--help`** by the question each answers: *selection* (which sessions match), *display* (how each match is shown), *limits* (how many survive), and for `scan`, *restore*. csb has three kinds of narrowing that had been rendering as one undifferentiated wall, and conflating the first two is exactly what caused the `--top` match-gate bug -- so the grouping is part of that fix rather than decoration. `--top` now sits visibly under *display*, labelled as never affecting what matches.

### Changed
- Requires **`dazzle-filekit>=0.4.2`** for `is_device_path`.
- **Existing indexes need `csb update rebuild-index`** to gain tool-derived folders. Re-indexing re-derives everything from the transcripts; until then a session's `tool_paths_extracted_at` stays NULL and its folders remain cwd-only.

### Tests
- **1235 pass, 1 skipped** (was 1089 at v0.7.0; +146).
- `tests/test_toolpaths.py` (55) covers the harvester the issue turned on: the four spellings of one folder, each of the five precision rules, the primary-resolution order, and the presence/work split. **Mutation-verified** -- each precision rule was removed on an isolated copy of the module to confirm the suite goes red. That exercise caught a test passing for the wrong reason: the obvious anchoring probe (`cat private/claude/x.md`) survives removal of the anchoring, because it is really testing the root allowlist. The test now uses a probe that fails without the anchor.
- Regression tests for the `--top` gate at **both** layers -- the query's behavior, and `cmd_scan`'s choice of what to pass it. The caller-level test reads the argument through the real signature rather than off `call_args.kwargs`, since it is passed positionally and a kwargs assertion would pass no matter what was sent. Verified red by re-introducing the regression.
- 3 new/rewritten metadata fixtures, verified **red against the pre-fix extractor** (the old ones modeled a world where `cwd` moved between unrelated directories -- real transcripts stopped doing that, which is why the bug survived). `tests/one-offs/probe_tool_path_harvest.py` ships as the reproducible population measurement.
...
**Onboarding overhaul: `csb setup`, protected-by-default, index-only as a signed exception (#52).** The 0.6.x theme. Found the hard way: after an RDP crash killed two sessions on a box whose rebuilt `~/.claude` was never `git init`-ed, every backup (manual + hooks) failed silently at the git precondition, the index stayed empty, and `csb list`/`csb scan` -- the exact commands built for that moment -- showed a bare "No sessions found." The transcripts were intact the whole time. The fix landed in two waves the same day: first recovery-first mechanics (repo-less indexing + diagnostic empty states), then the philosophical inversion -- csb's job is protection, so an unconfigured csb now actively drives you to `csb setup`, and running unprotected requires an explicit, recorded sign-off.

### Added (onboarding -- the 0.6.0 headline)
- **`csb setup`** -- guided onboarding walkthrough: resolves the Claude dir, detects an existing git repo (including one rooted at an ancestor, e.g. a home-dir repo tracking `.claude/` -- no nested repo is ever created over it), offers to `git init` when none exists, hardens `.gitattributes`, and offers/runs the first backup. **`csb setup --auto`** does all of it with zero prompts (scripts/provisioning). Non-TTY without a mode flag exits 2 with instructions instead of hanging. Closes with a **state-aware checklist** (`[x]`/`[ ]`) that probes real state -- the git store, the last indexed scan, and Claude Code's plugin registries (`installed_plugins.json` / `known_marketplaces.json`) -- and shows a command only for what's actually missing: setup never instructs you to run something that's already configured. Relocated setups (docker, VMs, host-mounts -- #45) are honored end-to-end: the dir comes from the usual precedence (`--claude-dir` > `CLAUDE_DIR` > `CLAUDE_CONFIG_DIR` > config > `~/.claude`) and the header names WHICH mechanism selected it (`via CLAUDE_CONFIG_DIR env`) before anything mutates.
- **`csb setup --index-only`** -- the ONLY sanctioned way to run unprotected: prints exactly what is given up, requires typing `index-only` back in interactive runs (the flag itself is the consent in scripted runs), and records `backup_mode: "index-only"` + an ack timestamp in the config. `csb setup` later re-enables protection and clears the sign-off loudly.
- **Every-run NO BACKUP PROTECTION banner.** A repo-less Claude dir without a recorded sign-off gets a loud stderr banner on EVERY interactive csb run -- unprotected-and-unacknowledged cannot be quiet. Resolved states (repo exists, or signed index-only) silence it; `--quiet` runs (hooks) and internal commands never show or consume it. This replaces the one-time first-run notice designed earlier the same day (never shipped).
- **`csb -V`** shorthand for `--version`, and **`csb help [command]`** now works as an alias for `--help` (was: `invalid choice: 'help'`).
- **Color.** `csb setup` and the no-protection banner use the same rich palette as `list`/`scan` when a real terminal is attached: green = done/protected, yellow = pending or the signed exception, red = problems and the banner headline, cyan = commands to run, dim = detail. Pipes, `--json` consumers, and rich-less installs get identical plain text.

### Fixed
- **`csb backup --no-commit` now works without a git repo** (was: hard error before any indexing, contradicting the README Quick Start). It scans + indexes everything -- unblocking `list`, `scan`, `search`, `resume`, `distill` -- skips gitattributes/commit phases, and warns loudly: `[index-only] Indexed N session(s) -- NO backup protection`. The warning survives `--quiet` (stderr), so hook logs can never mistake an index-only run for a backup. Bare `csb backup` still errors repo-less, and the error now names the `--no-commit` escape hatch alongside `git init`.
- **The SessionStart warning path works repo-less.** `csb _check` no longer hard-errors without a git repo (gap detection is DB-vs-disk only); its summary adapts to recommend `csb backup --no-commit` and flags that backups are failing. The hook can now surface the systemMessage warning on exactly the box that needs it most -- previously check errored, the hook fell to a "defensive backup" that also errored, and days of failures lived only in `csb-logs/`.
- **Hook fallback**: the backup hook probes for a git repo and falls back to `backup --no-commit` (logged as `[index-only]`) instead of spawning a run that dies at the precondition.
- **`python -m claude_session_backup` now propagates exit codes** (latent, found during #52): `__main__.py` called `main()` without `sys.exit`, so the module form always exited 0 -- the hook's PATH-less fallback would have read a `_check` gap (rc 10) as "all backed up". The `csb` console script was unaffected.

### Fixed (found by the v0.6.0 checklist run -- tester-unbounded agent)
- **`csb scan --json`** now exists (the render path always supported it; the parser never registered the flag). JSON stdout is pure in both directions: the `Scanning for...` preamble and empty-state prose are suppressed, and an empty result prints `[]`.
- **Hook log writes no longer clobber each other (Windows).** The spawned backup's log handle is now opened with kernel-level append semantics (`FILE_APPEND_DATA` via ctypes on Windows -- with a proper `HANDLE` restype -- `O_APPEND` on POSIX). A Python `"a"`-mode handle was NOT append-safe across process inheritance: the duplicated handle shares a file pointer snapshotted at open time, so a detached child overwrote notes appended after the open (reproduced: 1-of-3 `[index-only]` fallback notes surviving rapid fires, with mid-line truncation; now 3-of-3 intact). Spawn-site argument order also fixed (fallback note written before the handle opens), and the parent's fd is released after spawn.
- **Hook stdin tolerates PowerShell's BOM.** PowerShell's native `|` pipe prepends a UTF-8 BOM, which broke the hook's JSON parse and silently mis-filed the log as `backup-manual.log`. Stripped defensively; Claude Code's real Node hook runner was never affected.

### Added (recovery-first mechanics)
- **Diagnostic empty states.** `csb list` and `csb scan` distinguish "the index was never built" from "nothing matched": with transcripts on disk and an empty index they print `Note: the csb index is empty -- N session files exist on disk...` naming both backup forms; a path-scoped scan that misses while sessions exist elsewhere prints `Tip: N sessions exist under other folders -- try csb list -n 5...` (covers the "scanned my checkout, but the session's cwd was another folder" gap). Suppressed under `--json`/`--quiet`; deleted-scope queries are untouched.
- **`csb status` advice line** under `Git repo: NO`: `csb setup` to configure, and that `--no-commit` indexes without git. All repo-less messages (backup error, `[index-only]` warning, `_check` summary) converge on `csb setup` as the primary fix.
- `index.index_is_unbuilt()` + `scanner.count_session_jsonls()` -- the cheap primitives behind the empty-state diagnosis.

### Tests
- 46 new (1015 pass, was 969 at v0.5.1). The v0.6.0 human checklist was executed programmatically by an autonomous tester agent against isolated sandboxes (results: `tests/checklists/results/v0.6.0__results__2026-07-18.md`); its two FAIL findings became the fixes above, each pinned by a new regression test (scan `--json` purity x2, hook note-vs-spawn ordering, BOM'd stdin). Wave 1 (#52 mechanics): repo-less backup indexes / bare backup still errors / `--quiet` keeps the warning; list hint variants (hint, no-files, `--json` silence); `_empty_state_guidance` unbuilt/redirect/silent cases; `count_session_jsonls`; repo-less `_check` gap + clean; `csb status` advice; hook `--no-commit` fallback (SessionEnd + gap-warn paths) and with-repo plain spawn; module-form exit-code propagation (real subprocess). Wave 2 (onboarding): banner shows every run / silent with repo / silenced by signed ack / never on `--quiet` / JSON-stdout purity; `setup` parsing + `-V` == `--version`; `cmd_setup` end-to-end with real `git init` (`--auto` init+backup, interactive accept/decline/EOF, non-TTY exit 2, `--index-only` flag + typed-confirmation flows, repo-present ack clearing, index-only-with-repo refusal, missing dir); state-aware checklist (all-done shows zero commands, marketplace-only shows just the install line, declined backup stays `[ ]`). One behavior test updated (`_check` repo-less no longer rc 1), one made wrap-immune. New `mock_claude_dir_repoless` conftest fixture.
