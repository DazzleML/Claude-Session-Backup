...
csb setup schedule                    # Install an OS-scheduled backup (guided interval question)
csb setup schedule --auto             # Same, no prompts (24h interval)
csb setup schedule --interval 720     # Explicit cadence in minutes (15/720/1440 presets)
csb setup schedule --dry-run          # Print the exact task XML / crontab block / plist; change nothing
csb setup schedule --status           # Health: OS readback + run-log evidence ("INSTALLED BUT NOT RUNNING")
csb setup schedule --remove           # Remove the csb entry (only the csb entry; snapshots first)
csb setup schedule --print-systemd    # Recipe for cron-less Linux (csb doesn't manage systemd units)
...
**The roster is re-derived — and now durable.** Membership comes from each session's real activity *timeline* (every sitting recorded in the index), not just its last heartbeat, so **reclaiming a session no longer erases its past**: resume it today and it still appears in every epoch it actually touched, because those old sittings stay on record. A session belongs to `last~4` and to `last~1` if it was genuinely active in both — and to nothing in between if it sat closed there. Come back a week later and `csb set show last~4` gives the same answer. (Sessions indexed before segments existed, or purged before ever being scanned, fall back to the old last-activity rule until a `csb backup` or `csb update rebuild-index` backfills their timeline — and where a boundary snapshot proves such a session was open, it is appended to the roster with an explicit `[open at shutdown -- no indexed activity in this window]` mark.) If you want to *curate* a group rather than observe one, promote it to a named set.
